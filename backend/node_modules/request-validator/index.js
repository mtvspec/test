...
var RequestValidator = {
  validate: function validate(data, type) {
    ...
    switch (type) {
      case 'iin':
      return iinValid(data);
      break;
      case 'person':
      return validators.validatePerson(data);
      break;
      case 'personData':
      return validators.validatePersonData(data);
      break;
      case 'company':
      return validators.validateCompany(data);
      break;
      case 'companyData':
      return validators.validateCompanyData(data);
      break;
      default:
...
var validators = {
  validatePerson: function validatePerson(p) {
    ...
    if (!p.middleName) {
      return (
        !(p.id) ? {result: false, message: 'no person id'}
        : !(iin.result) ? {result: iin.result, message: iin.message}
        : !(p.lastName) ? {result: false, message: 'no person last name'}
        : !(lastName.result) ? {result: lastName.result, message: lastName.message}
        : !(p.firstName) ? {result: false, message: 'no person first name'}
        : !(firstName.result) ? {result: firstName.result, message: firstName.message}
        : !(p.dob) ? {result: false, message: 'no person dob'}
        : !(dob.result) ? {result: dob.result, message: dob.message}
        : !(p.genderID) ? {result: false, message: 'no person gender id'}
      ...
      return (
        ...
        : !(p.firstName) ? {result: false, message: 'no person first name'}
        : !(firstName.result) ? {result: firstName.result, message: firstName.message}
        : !(p.middleName) ? {result: false, message: 'no person middle name'}
  ...
  validatePersonData: function validatePersonData(p) {
    if (!(typeof p === 'object')) {
      return (
        {result: false, message: 'no person data'}
      );
    }
    if (!(Object.keys(p).length === 5)) {
      return (
        {result: false, message: 'person data must have at 5 properties'}
      )
    }
    var person = {
      lastName: String(p.lastName),
  ...
  validateCompany: function validateCompany(c) {
    if (!(typeof c === 'object')) {
      return (
        {result: false, message: 'no company'}
      );
    }
    if (!(Object.keys(c).length === 4)) {
      return (
        {result: false, message: 'company must have at 4 properties'}
      )
    }
    var company = {
      id: String(c.id),
      shortName: String(c.shortName),
      longName: String(c.longName),
      fullName: String(c.fullName)
    },
    bin = (iinValid(company.id)),
    shortName = companyNameValid(company.shortName, 2, 25),
    longName = companyNameValid(company.longName, 2, 50),
    fullName = companyNameValid(company.fullName, 2, 100);
    if (!(company.longName && company.fullName)) {
      return (
        !(company.id) ? {result: false, message: 'no company id'}
        : !(bin.result) ? {result: bin.result, message: bin.message}
        : !(company.shortName) ? {result: false, message: 'no company short name'}
        : !(shortName.result) ? {result: lastName.result, message: shortName.message}
        : {result: true, message: 'company with iin "'+company.id+'" data valid'}
      );
      ...
      return (
        ...
        : !(bin.result) ? {result: bin.result, message: bin.message}
        : !(company.shortName) ? {result: false, message: 'no company short name'}
        : !(shortName.result) ? {result: shortName.result, message: shortName.message}
        : !(company.longName) ? {result: false, message: 'no company long name'}
        : !(longName.result) ? {result: longName.result, message: longName.message}
        : {result: true, message: 'company with iin "'+company.id+'" data valid'}
      );
    } else {
      return (
        ...
        : !(company.longName) ? {result: false, message: 'no company long name'}
        : !(longName.result) ? {result: longName.result, message: longName.message}
        : !(company.fullName) ? {result: false, message: 'no company full name'}
        : !(fullName.result) ? {result: fullName.result, message: fullName.message}
        : {result: true, message: 'company with iin "'+company.id+'" data valid'}
      );
    }
  },
  validateCompanyData: function validateCompanyData(c) {
    if (!(typeof c === 'object')) {
      return (
        {result: false, message: 'no company data'}
      );
    }
    if (!(Object.keys(c).length === 3)) {
      return (
        {result: false, message: 'company data must have at 3 properties'}
      )
    }
    var company = {
      shortName: String(c.shortName),
...
function companyNameValid(text, minLength, maxLength) {
  return (
    ...
    : !(text.length <= maxLength) ? {result: false, message: 'name '+text+' must have length equals "'+maxLength+'" or have length less than "'+maxLength+'" letters'}
