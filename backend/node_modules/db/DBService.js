...
DBService.prototype.performQuery = function performQuery(query, log, queryFn, cb) {
  ...
  for (key in log.values) {
    if (log.values.hasOwnProperty(key)) {
      placeholders[key] = '$' + counter;
      replacements.push(log.values[key]);
      counter++;
    }
  }
  ...
  log = {
    text: format(log.text, placeholders),
    values: replacements
  };
