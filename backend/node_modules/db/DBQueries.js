var DBQueries = {
  selectProjectsQuery: function selectProjectsQuery() {
    return {
      text: 'SELECT id, customer_id AS "customerID", project_formal_name AS "projectFormalName", project_work_name AS "projectWorkName", project_official_name AS "projectOfficialName", start_date AS "startDate", end_date AS "endDate", budget, manager_id AS "managerID" FROM e_projects WHERE is_deleted = \'N\' ORDER BY id ASC'
    }
  },
  ...
  selectPersonsQuery: function selectPersonsQuery() {
    return {
      text: 'SELECT id, last_name AS "lastName", first_name AS "firstName", middle_name AS "middleName", dob, gender_id AS "genderID", is_deleted AS "isDeleted" FROM e_persons ORDER BY id ASC'
    }
  },
  ...
  selectPersonQuery: function selectPersonQuery(person) {
    return {
      text: 'SELECT id, last_name AS "lastName", first_name AS "firstName", middle_name AS "middleName", dob, gender_id AS "genderID", is_deleted AS "isDeleted" FROM e_persons WHERE id = {id}',
      values: person
    }
  },
  insertPersonQuery: function insertPersonQuery(person) {
    return {
    text: 'INSERT INTO e_persons (id, last_name, first_name, middle_name, dob, gender_id) VALUES ({id}, {lastName}, {firstName}, {middleName}, {dob}, {genderID}) RETURNING id',
    values: person
    }
  },
  ...
  selectCompaniesQuery: function selectCompaniesQuery() {
    return {
      text: 'SELECT id, company_name AS "companyName" FROM e_companies WHERE is_deleted = \'N\' ORDER BY id ASC'
    }
  },
  ...
  insertCompanyQuery: function insertCompanyQuery(company) {
    return {
      text: 'INSERT INTO e_companies (id, company_name) VALUES ({id}, {companyName}) RETURNING id',
      values: company
    }
  },
  selectUserQuery: function selectUserQuery(user) {
    return {
      text: 'SELECT id, person_id AS "personID", status FROM meta.e_users WHERE u_username = {username};',
      values: user
    }
  }
};
