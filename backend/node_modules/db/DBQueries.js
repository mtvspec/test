var DBQueries = {
  ...
  insertProjectQuery: function insertProjectQuery(project) {
    return {
      text: 'INSERT INTO project.e_projects (customer_id, project_formal_name, project_work_name, project_official_name, start_date, end_date, budget, manager_id) VALUES ({projectCustomerID}, {projectFormalName}, {projectWorkName}, {projectOfficialName}, {projectStartDate}, {projectEndDate}, {projectBudget}, {projectManagerID}) RETURNING id;',
      values: project
    }
  },
  ...
  selectPersonsQuery: function selectPersonsQuery() {
    return {
      text: 'SELECT id, last_name AS "lastName", first_name AS "firstName", middle_name AS "middleName", dob, gender_id AS "genderID" FROM person.e_persons WHERE is_deleted = \'N\' ORDER BY id ASC;'
    }
  },
  ...
  selectPersonQuery: function selectPersonQuery(person) {
    return {
      text: 'SELECT id, last_name AS "lastName", first_name AS "firstName", middle_name AS "middleName", dob, gender_id AS "genderID" FROM person.e_persons WHERE is_deleted = \'N\' AND id = {id};',
      values: person
    }
  },
  ...
  selectCompaniesQuery: function selectCompaniesQuery() {
    return {
      text: 'SELECT id, short_name AS "shortName", long_name AS "longName", full_name AS "fullName" FROM company.e_companies WHERE is_deleted = \'N\' ORDER BY id ASC;'
    }
  },
  ...
  selectUserRolesByUserID: function selectUserRolesByUserID(user) {
    return {
      text: 'SELECT id, user_id AS "userID", role_id AS "roleID" FROM meta.r_e_user_e_roles WHERE is_deleted = \'N\' AND user_id = {userID};',
      values: user
    }
  },
  ...
  selectUserRoles: function selectUsersRoles(user) {
    return {
      text: 'SELECT u.user_id AS "userID", r.role_name AS "roleName" FROM meta.r_e_user_e_roles u, meta.e_roles r WHERE u.role_id = r.id AND {userID} ASC;',
      values: user
    }
  },
  selectUsersRolesQuery: function selectUsersRolesQuery() {
    return {
      text: 'SELECT u.user_id AS "userID", r.role_name AS "roleName" FROM meta.r_e_user_e_roles u, meta.e_roles r WHERE u.role_id = r.id ORDER BY r.id ASC'
    }
  },
