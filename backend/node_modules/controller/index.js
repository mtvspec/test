...
Controller.prototype.validate = function validate(data, type) {
  ...
  case 'bin':
    ...
    break;
  case 'company':
    if (!(data.id && data.shortName, data.longName, data.fullName)) {
      return false;
    }
    if (!binValid(data.id)) {
      console.log('bin');
    ...
    if (!nameValid(data.shortName, 1, 500)) {
      console.log('short name');
      return false;
    }
    if (!nameValid(data.longName, 1, 1000)) {
      console.log('long name');
      return false;
    }
    if (!nameValid(data.fullName, 1, 2000)) {
  ...
  case 'user':
    ...
    if (!nameValid(data.password, 6, 20)) {
      return false;
    }
    return true;
  case 'projectData':
  ...
  case 'projectResult':
    ...
    if (!(data.projectID, data.typeID)) {
      return false;
    }
    if (!idValid(data.projectID)) {
      return false;
    }
    if (!idValid(data.typeID)) {
      return false;
    }
    if (data.resultName) {
      if (!nameValid(data.resultName, 1, 500)) {
        return false;
      }
    }
    if (data.parentID) {
      if (!idValid(data.parentID)) {
        return false;
      }
    }
    if (data.authorID) {
      if (!iinValid(data.authorID)) {
        return false;
      }
    }
    if (data.responsiblePersonID) {
      if (!iinValid(data.responsiblePersonID)) {
        return false;
      }
    }
    if (data.statusID) {
      if (!idValid(data.statusID)) {
        return false;
      }
    }
    return true;
  case 'remarkData':
    ...
    if (!idValid(data.objectID)) {
      return false;
    }
    if (!nameValid(data.remarkText, 1, 4000)) {
      return false;
    }
    if (!iinValid(data.responsiblePersonID)) {
      return false;
...
function genderIDValid(gender) {
  if (typeof gender === 'string') {
    ...
    console.log('Invalid genderID:', gender);
    return false;
  }
};
