...
.get('/', function (request, response) {
  var res;
  PGService.performQueryWithoutLogging(DBQueries.selectPersonsQuery(), PGService.selectRecords, function (status, data) {
...
.put('/:iin', function (request, response) {
  ...
  console.log('Update:\n', person);
  if (PGController.validate(person, 'personData')) {
    PGService.performQuery(DBQueries.updatePersonQuery(person), DBQueries.insertPersonQueryLog(personLog), PGService.updateRecord, function (status, data) {
      res = response.status(status);
      if (status === 200) {
        res.json(data);
      }
