'use strict';

var pg = require('pg');

function LTService() {
};

LTService.prototype.data = [];

LTService.prototype.getRecords = function getRecords(query) {
  return new Promise(function(resolve, reject) {
    if (!query) {
      reject({
        status: 400,
        message: 'query not found'
      });
    }
    if (!query.text) {
      reject({
        status: 400,
        message: 'query text not found'
      });
    }
    pg.connect(function (err, client, done) {
      if (err) {
        console.error('selectRecords:', err.detail);
        reject({
          status: 500,
          message: err.detail
        });
      } else {
        client.query({
          text: query.text
        }, function (err, result) {
          var rowCount;
          if (err) {
            console.error('selectRecords:', err.detail);
            reject({
              status: 500,
              message: err.detail
            });
          } else {
            done();
            rowCount = result.rowCount;
            if (rowCount === 0) {
              resolve({
                status: 204,
                data: []
              });
            } else if (rowCount > 0) {
              resolve({
                status: 200,
                data: result.rows
              });
            }
          }
        });
      }
    });
  });
};

LTService.prototype.getRecord = function getRecord(query) {
  return new Promise(function(resolve, reject) {
    if (!query) {
      console.log('query not found');
      reject({
        status: 400,
        message: 'query not found'
      });
    }
    if (!query.text) {
      console.log('query text not found');
      reject({
        status: 400,
        message: 'query text not found'
      });
    }
    if (!query.values) {
      console.log('query values not found');
      reject({
        status: 400,
        message: 'query values not found'
      });
    }
    pg.connect(function (err, client, done) {
      if (err) {
        console.error('selectRecord:', err.detail);
        reject({
          status: 500,
          message: err.detail
        });
      } else {
        client.query({
          text: query.text,
          values: query.values
        }, function (err, result) {
          var rowCount;
          if (err) {
            console.error('selectRecord:', err.detail);
            reject({
              status: 500,
              message: err.detail
            });
          } else {
            done();
            rowCount = result.rowCount;
            if (rowCount === 0) {
              resolve({
                status: 204,
                data: []
              });
            } else if (rowCount === 1) {
              resolve({
                status: 200,
                data: result.rows[0]
              });
            }
          }
        });
      }
    });
  });
};

LTService.prototype.insertRecord = function insertRecord(query) {
  return new Promise(function(resolve, reject) {
    if (!query) {
      reject({
        status: 400,
        message: 'query not found'
      });
    }
    if (!query.text) {
      reject({
        status: 400,
        message: 'query text not found'
      });
    }
    if (!query.values) {
      reject({
        status: 400,
        message: 'query values not found'
      });
    }
    pg.connect(function (err, client, done) {
      if (err) {
        console.error('insertRecord:', err.detail);
        reject({
          status: 500,
          message: err.detail
        });
      } else {
        client.query({
          text: query.text,
          values: query.values
        }, function (err, result) {
          var rowCount;
          if (err) {
            console.error('insertRecord:', err.detail);
            if (err.code === '23505') {
              reject({
                status: 400,
                message: err.detail
              });
            }
            if (err.code === '23502') {
              reject({
                status: 400,
                message: err.column
              });
            }
            reject({
              status: 500,
              message: err.detail
            });
          } else {
            done();
            rowCount = result.rowCount;
            if (rowCount === 0) {
              resolve({
                status: 204,
                data: []
              });
            } else if (rowCount === 1) {
              resolve({
                status: 201,
                data: result.rows[0]
              });
            }
          }
        });
      }
    });
  });
};

LTService.prototype.updateRecord = function updateRecord(query) {
  return new Promise(function(resolve, reject) {
    if (!query) {
      reject({
        status: 400,
        message: 'query not found'
      });
    }
    if (!query.text) {
      reject({
        status: 400,
        message: 'query text not found'
      });
    }
    if (!query.values) {
      reject({
        status: 400,
        message: 'query values not found'
      });
    }
    pg.connect(function (err, client, done) {
      if (err) {
        console.error('updateRecord:', err.detail);
        reject({
          status: 500,
          message: err.detail
        });
      } else {
        client.query({
          text: query.text,
          values: query.values
        }, function (err, result) {
          var rowCount;
          if (err) {
            console.error('updateRecord:', err.detail);
            reject({
              status: 500,
              message: err.detail
            });
          } else {
            done();
            rowCount = result.rowCount;
            if (rowCount === 0) {
              resolve({
                status: 204,
                data: []
              });
            } else if (rowCount === 1) {
              resolve({
                status: 200,
                data: result.rows[0]
              });
            }
          }
        });
      }
    });
  });
};

LTService.prototype.deleteRecord = function deleteRecord(query) {
  return new Promise(function(resolve, reject) {
    if (!query) {
      reject({
        status: 400,
        message: 'query not found'
      });
    }
    if (!query.text) {
      reject({
        status: 400,
        message: 'query text not found'
      });
    }
    if (!query.values) {
      reject({
        status: 400,
        message: 'query values not found'
      });
    }
    pg.connect(function (err, client, done) {
      if (err) {
        console.error('deleteRecord:', err.detail);
        reject({
          status: 500,
          message: err.detail
        });
      } else {
        client.query({
          text: query.text,
          values: query.values
        }, function (err, result) {
          var rowCount;
          if (err) {
            console.error('deleteRecord:', err.detail);
            reject({
              status: 500,
              message: err.detail
            });
          } else {
            done();
            rowCount = result.rowCount;
            if (rowCount === 0) {
              resolve({
                status: 204,
                data: []
              });
            } else if (rowCount === 1) {
              resolve({
                status: 200,
                data: result.rows[0]
              });
            }
          }
        });
      }
    });
  });
};

module.exports = LTService;
